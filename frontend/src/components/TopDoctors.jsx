...
const TopDoctors = () => {
  ...
  return (
    <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
      ...
      <p className='text-sm w-1/3 text-center'>Simply browse through our extensive list of trusted doctors.</p>
      <div className='w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
        {doctors.slice(0,10).map((item,index)=>(
          <div onClick={()=>{navigate(`/appointment/${item._id}`); window.scroll(0,0)}} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={index}>
            ...
            <p className='w-2 h-2 bg-red-500 rounded-full'></p><p>Available</p>
              </div>
      ...
      <button onClick={()=>{ navigate('/doctors'); window.scroll(0,0) }} className='bg-green-100 text-black px-12 py-3 rounded-full mt-10 font-medium'>more</button>
    </div>
  )
}
